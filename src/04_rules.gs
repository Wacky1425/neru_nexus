...
function calculateRequiredGoalCash_(monthlyFreeCash) {
  ...
  for (const goal of goals) {
    ...
    const monthlyCapacity = Math.max(1, Number(monthlyFreeCash || 0));

    const requiredSavingMonths = Math.ceil(remainingAmount / monthlyCapacity);

    // 相場リスク用の余裕。
    // 必要時期の6か月前には現金化完了を目指す。
    const safetyMonths = 6;

    const cashPreparationStartMonths = requiredSavingMonths + safetyMonths;

    const shouldReserveNow = remainingMonths <= cashPreparationStartMonths;

    let requiredCashNow = 0;

    if (shouldReserveNow) {
      const monthsAvailableForSaving = Math.max(
        1,
        remainingMonths - safetyMonths,
      );

      const amountThatCanStillBeSaved =
        monthlyCapacity * monthsAvailableForSaving;

      requiredCashNow = Math.max(
        0,
        remainingAmount - amountThatCanStillBeSaved,
      );
    }

    requiredCash += requiredCashNow;
...
function calculateMonthlyFreeCash_(budgets, expenses, projectedIncome) {
  ...
  const minimumNisa = Number(budgets["NISA積立"] || 0);
...
function calculateGoalMonthlyAllocation_(goals, availableAmount, settings) {
  ...
  for (const goal of sortedGoals) {
    ...
    const allocated = Math.min(requiredThisMonth, remainingCapacity);
    ...
    details.push({
      ...
      shortageThisMonth: Math.max(0, requiredThisMonth - allocated),

      onTrack: allocated >= requiredThisMonth,
    });
  }
