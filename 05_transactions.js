...
function buildDuplicateKey(tx) {
  return [
    normalizeDuplicateSource_(tx.source_type),
    normalizeDuplicateDate_(tx.transaction_date),
    normalizeDuplicateAmount_(tx.amount),
    normalizeDuplicateText_(tx.merchant)
  ].join("|");
}

function normalizeDuplicateSource_(value) {
  return String(value || "")
    .normalize("NFKC")
    .trim();
}
...
function rebuildAllDuplicateKeys() {
  ...
  const duplicateKeyColumn =
    table.index["duplicate_key"] + 1;

  const output = table.rows.map(
    row => {
      const tx = {
        source_type:
          row[
  ...
  Logger.log(
    `${output.length}件のduplicate_keyを再生成しました`
  );
}

function applyClassificationToReviewMerchant_(
  merchant,
  classification
) {
  const targetMerchant = String(
    merchant || ""
  ).trim();
Logger.log(
  ...
  for (
    ...
    Logger.log(
  `row=${rowIndex + 1} ` +
  `rowMerchant=[${rowMerchant}] ` +
  `status=[${status}]`
);

    if (rowMerchant !== targetMerchant) {
      continue;
    }
  ...
  `★ 一致: row=${rowIndex + 1}`
    ...
    row[index["major_category"]] =
      classification.majorCategory;

    row[index["sub_category"]] =
      classification.subCategory;

    row[index["purpose_type"]] =
      classification.purposeType;

    row[index["expense_ratio"]] =
      expenseRatio;

    row[index["expense_amount"]] =
      amount * expenseRatio;
