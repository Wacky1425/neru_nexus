...
function updateTransactionFromApp_(data) {
  console.log(
    "★★ updateTransactionFromApp_ START ★★"
  );
  const id = String(
    data.id || ""
  ).trim();
...
const ruleMerchant = String(
  ...
  // duplicate_keyは取込時の取引識別子なので、
// 編集・要確認処理では変更しない
let duplicateKey = getString(
  existingRow,
  table.index,
  "duplicate_key"
).trim();

// 古いデータなどでduplicate_keyが空の場合だけ、
// 「編集前の元データ」から生成する
if (!duplicateKey) {
  duplicateKey = buildDuplicateKey({
    source_type: getString(
      existingRow,
      table.index,
      "source_type"
    ),
...
if (saveRule) {
  const merchantForRule =
    ...
    getString(
     ...
     Logger.log(
    `merchantForRule=[${merchantForRule}]`
  );
console.log(
  "★★ saveRuleブロック突入 ★★"
);
  ...
  // 新規追加でも既存ルールでも、
  // 残っている同一merchantの要確認へ適用
  appliedReviewCount =
  applyClassificationToReviewMerchant_(
    merchantForRule,
    {
      type,
      majorCategory,
